...
export default function AdminLaporan() {
  ...
  useEffect(() => {
    fetchReports()
    supabase.from('zones').select('*').order('name').then(({ data }) => setZones(data ?? []))
  }, [fetchReports])

  useEffect(() => {
    const channel = supabase
      .channel('admin-laporan')
      ...
      if (report?.user_id) {
        await supabase.from('notifications').insert({
          ...
          message: newStatus === 'in_progress'
            ? `Laporan plat ${report.plate_number} sedang ditangani`
            ...
            return (
              <div key={report.id} className="bg-white rounded-2xl border border-slate-200 overflow-hidden">
                <div className="flex gap-3 p-4">
                  {report.photo_url && (
                    <img src={report.photo_url} alt="" className="w-20 h-20 rounded-xl object-cover shrink-0 cursor-pointer"
                      onClick={() => setSelectedReport(report)} />
                  )}
                  <div className="flex-1 min-w-0">
                    <div className="flex items-start justify-between gap-2 mb-2">
                      <div className="bg-slate-900 text-white font-mono text-xs px-2.5 py-1.5 rounded-lg tracking-widest">
                        {report.plate_number ?? '?????'}
                      </div>
                ...
                {report.status !== 'resolved' && (
                  <div className="px-4 pb-3 flex gap-2">
                    {report.status === 'pending' && (
                      <button onClick={() => handleUpdateStatus(report.id, 'in_progress')}
                    ...
                    {report.status === 'in_progress' && (
                      <button onClick={() => handleUpdateStatus(report.id, 'resolved')}
                        disabled={updating === report.id}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center px-4 bg-black/70"
          ...
          <div className="w-full max-w-lg bg-white rounded-2xl overflow-hidden shadow-2xl"
            ...
            {selectedReport.photo_url && (
              <img src={selectedReport.photo_url} alt="" className="w-full max-h-80 object-cover" />
            )}
            <div className="p-4">
              <div className="flex items-center justify-between mb-2">
                <div className="bg-slate-900 text-white font-mono text-sm px-3 py-1.5 rounded-lg tracking-widest">
                  {selectedReport.plate_number ?? '?????'}
                </div>
                <span className={`text-xs font-medium px-2.5 py-1 rounded-full border ${statusConfig[selectedReport.status]?.color}`}>
              ...
              <button onClick={() => setSelectedReport(null)}
                className="mt-4 w-full bg-slate-100 hover:bg-slate-200 text-slate-700 font-semibold py-2.5 rounded-xl transition text-sm">
                Tutup
              </button>
            </div>
          </div>
