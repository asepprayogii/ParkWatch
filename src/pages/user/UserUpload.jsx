...
export default function UserUpload() {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault()
    if (!photo) { setError('Foto kendaraan wajib diupload'); return }
    if (!form.plate_number) { setError('Nomor plat wajib diisi'); return }
    if (!form.zone_id) { setError('Pilih zona parkir'); return }
      ...
      <form onSubmit={handleSubmit} className="py-3 space-y-5">
        ...
        {!ocrLoading && photo && (
          <>
            ...
            <div>
              <Input
                label={`Nomor Plat ${ocrFailed ? '(Isi Manual)' : '(Terdeteksi Otomatis)'}`}
                name="plate_number"
            ...
            <div>
              ...
              {zones.length === 0 && (
                <p className="text-xs text-slate-400 mt-1">Belum ada zona — hubungi admin</p>
              )}
            </div>
