...
export default function Sidebar({ onCollapse }) {
  ...
  return (
    <>
      {/* Desktop Sidebar */}
      <aside 
        className="hidden md:flex flex-col fixed left-0 top-0 h-full bg-white border-r border-slate-200 z-50 transition-all duration-300"
      ...
      >
        ...
        <div className={`flex items-center h-14 border-b border-slate-200 px-3 ${collapsed ? "justify-center" : "justify-between"}`}>
          {!collapsed && (
            <div className="flex items-center gap-2 overflow-hidden">
              ...
              <span className="font-bold text-slate-800 text-sm whitespace-nowrap">ParkWatch</span>
            </div>
          )}
          <button 
            onClick={handleToggle} 
            className="w-7 h-7 flex items-center justify-center rounded-lg hover:bg-slate-100 text-slate-500 transition shrink-0" 
        ...
        <nav className="flex-1 px-2 py-3 flex flex-col gap-1 overflow-y-auto">
          {navItems.map((item) => {
            ...
            return (
              ...
              >
                {/* Icon dengan badge dot (collapsed) atau angka (expanded) */}
                ...
                {!collapsed && (
                  <>
                    ...
                    {showBadge && (
                      <span className="bg-red-500 text-white text-[10px] font-bold px-1.5 py-0.5 rounded-full min-w-[18px] h-4.5 flex items-center justify-center shrink-0">
                        {displayBadge}
                      </span>
                    )}
                  </>
        ...
        <div className="px-2 py-3 border-t border-slate-100">
          <button 
            ...
            className={`w-full flex items-center gap-3 px-2 py-2.5 rounded-xl text-sm font-medium text-red-500 hover:bg-red-50 transition ${collapsed ? "justify-center" : "justify-start"}`}
          >
            ...
            return (
              ...
              >
                <div className="relative">
                  {item.icon}
                  {showBadge && (
                    <span className="absolute -top-1 -right-1 min-w-[16px] h-4 bg-red-500 text-white text-[8px] font-bold rounded-full flex items-center justify-center px-1 border-2 border-white">
                      {displayBadge}
                    </span>
                  )}
                </div>
